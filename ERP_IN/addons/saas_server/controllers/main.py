...
class SaasServer(http.Controller):
    ...
    @http.route('/odoo/test', type='http',
                auth='public', website=True,cors="*", csrf=False)
    def index(self, **args):
        _logger.info('CONNECTION SUCCESSFUL')
        _logger.info(args)
        client_id= args.get('client_id','12345678')
        client_data = {
            'name': 'test',
            'client_id': client_id,
            'expiration_datetime': '01/02/2018',
            'trial': 9,
            'host': 'bave.io',
        }
        client = request.env['saas_server.client'].sudo().search([('client_id', '=', client_data.get('client_id'))])
        if not client:
    ...
    # http://127.0.0.1:9999/saas_server/new_database?oauth_access_token=aghcvx000kkk&u=user&d=db3&h=bave.io&e=&expiration_datetime_user=2018-08-28%2009:30:00&trial=15&demo=false&addons=&lang=vi_VN&db_template=10_agara_template&original_login=admin_phamhung@bave.io&login=admin&password=matkhau&mobile=06945165&email=haha@yahoo.com

    # http://127.0.0.1:9999/saas_server/new_database?
    # u=user                                     // sua ten cua admin trong db moi tao
    # &d=dumia                                   //ten db moi
    # &h=bave.io                                 //luu vao ban ghi db saas_server.client
    # &e=ngay%20het%20han                        //datetime luu vao ban ghi saas_server.client
    # &expiration_datetime_user=2018-08-28%2009:30:00  //ngay het han cua user
    # &trial=false                               //luu vao ban ghi db saas_server.client
    # &demo=false                                // input master data demo khi tao db
    # &addons=                                   //cac module se cai sau khi create db (chi dung khi tao moi db~ khong truyen param db_template)
    # &lang=vi_VN                                //lang khi tao db moi
    # &db_template=10_test                       //template db
    # & original_login = admin_phamhung @ bave.io //user template da ton tai trong db_template, sua lai user nay = username, password truyen vao => dung de gui cho khach hang
    # &login=userlogin                           //thay doi login cua user admin trong db moi
    # &password=matkhau                          //thay doi password cua user admin trong db moi
    # &mobile=06945165                           // sua mobile cua admin trong db moi
    # &email=haha@yahoo.com                      // sua email cua admin gtrong db moi
    # &oauth_access_token=                       // token code (post)
    #
    # db phai khac db hien tai


    @http.route('/saas_server/new_database', type='http', website=True,  auth='public',cors="*", csrf=False)
    def new_database(self, **state):
        oauth_access_token = state.get('oauth_access_token') or ''
        ...
        if not user:
            return simplejson.dumps({
                'response': 'NOK', 'errorcode': 'token code invalid !'})

        _logger.info('new_database post: %s', state)

        owner_user = state.get('u')
        new_db = state.get('d')
        host = state.get('h')
        trial = state.get('t')
        expiration_datetime = state.get('e') or False
        expiration_datetime_user = state.get('expiration_datetime_user', False) or False
...
#        tz = state.get('tz')
   ...
   #     client_id=state.get('client_id')
        original_login = state.get('original_login', False)
        login = state.get('login')
        password = state.get('password')
        mobile = state.get('mobile')
        email = state.get('email')

        # if not expiration_datetime_user:
        #     trail_hour = 24
        #     expiration_datetime_user = \
        #         datetime.datetime.now() + \
        #         datetime.timedelta(hours=trail_hour)

        client_data = {
            'name': new_db,
            'client_id': new_db,
            'expiration_datetime': expiration_datetime,
            'expiration_datetime_user': expiration_datetime_user,
            'trial': trial,
            'host': host,
        }

        if isinstance(addons, (unicode, str)):
            addons = addons.split(',')

        try:
            client = request.env['saas_server.client'].sudo().search([('client_id', '=', new_db)])
            if not client:
                client = request.env['saas_server.client'].sudo().create(client_data)
            client = client[0]
            ...
            partner_id= client.updateLoginInfo(
                login, password, expiration_datetime_user,
                original_login)

            client.updatePartnerInfo(partner_id,owner_user,mobile,email)
            return simplejson.dumps({
            'response': 'OK', 'errorcode': "operation is successfuly."})
        except Exception as ex:
            return simplejson.dumps({
            'response': 'NOK','errorcode':ex.message})

    @http.route('/saas_server/new_database_old', type='http', website=True, auth='public')
    def new_database_old(self, **post):
