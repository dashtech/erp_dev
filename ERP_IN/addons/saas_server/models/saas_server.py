...
class SaasServerClient(models.Model):
    ...
    expiration_datetime_user = fields.Datetime(readonly=True)
    ...
    @api.one
    def updatePartnerInfo(self,partner,owner_user,mobile=None,email=None):
        try:
            registry = self.registry()[0]
            with registry.cursor() as client_cr:
                client_env = api.Environment(client_cr, SUPERUSER_ID, self._context)
                partners = client_env['res.partner'].browse(partner.id)
                if not partners:
                    raise Exception("Could'n find partner with id %s " % (partner.id))
                blPartner=partners.write({'name':owner_user,'mobile': mobile, 'email': email})
                return blPartner
        except Exception as e:
            raise Exception('An error happened during update userinfo %s ' % (e.message))

    @api.multi
    def updateLoginInfo(self, login, password,
                        expiration_datetime_user,
                        original_login=False):
        try:
            registry = self.registry()[0]
            with registry.cursor() as client_cr:
                client_env = api.Environment(client_cr, SUPERUSER_ID, self._context)
                if original_login:
                    user = client_env['res.users'].search(
                        [('login', '=', original_login)])
                else:
                    user = client_env['res.users'].search([('id', '=', 1)])
                if not user:
                    raise Exception("Could'n find user with login %s " % (original_login))
                user[0].write({'login': login, 'password': password,
                               'expiration_datetime': expiration_datetime_user})
    ...
    def action_delete_database(self):
        for client in self:
            ...
            name = u'{}_remove_{}'.format(client.name, datetime.now())
