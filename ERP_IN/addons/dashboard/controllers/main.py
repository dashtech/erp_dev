...
class Dashboard(http.Controller):
    ...
    def get_chart_data(self,chart):
        ...
        elif chart.chart_date_field_id.ttype in ['char', 'integer', 'float']:
            mesure = sum_count + chart.chart_measure_field_id.name + ") as mesure"
            ykey = chart.chart_date_field_id.name + " as date "
        ...
        if not group_by:
            ...
            if chart.chart_date_field_id.ttype == 'datetime':
                group_by = "date(" + group_by + ")"
        requete+=" Group by " + group_by
        cr.execute(requete)
        query_result = cr.dictfetchall()
        res = sorted(query_result, key=itemgetter('date'), reverse=False)
        return res
