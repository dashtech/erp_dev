...
class VoucherVoucher(models.Model):
	...
	validity = fields.Integer(string='Validity(in days)',default=_get_default_validity, help="Validity of this Voucher in days")
	...
	customer_id = fields.Many2one(comodel_name='res.partner', string='Created For', help='Optional: The cart rule will be available to everyone if you leave this field blank.',domain=[('customer','=',True)])
	...
	def create(self, vals):
		...
		if vals['total_available'] == 0:
			raise  ValidationError(_('Total Availability can`t be 0. Choose -1 for unlimited or greater than 0 !!!'))
		if vals['voucher_value'] < default_values['min_amount']:
			raise  ValidationError(_('You can`t create voucher below this minimum amount (%s) !!!')%default_values['min_amount'])
		if vals['voucher_value'] > default_values['max_amount']:
			raise  ValidationError(_('You can`t create voucher greater than this maximum amount (%s) !!!')%default_values['max_amount'])
		if vals.get('voucher_value') <= 0:
			raise  ValidationError(_('Voucher Value cannot be <= 0.'))
		...
		if not vals.get('voucher_code'):
			...
			if vals['voucher_code'] == '':
				raise  ValidationError(_('Code can not contain empty spaces'))
		...
		if str(vals.get('expiry_date')) < vals.get('issue_date'):
			raise  ValidationError(_('Applicable date should be less than expiry date.'))
		...
		if vals.get('expiry_date') and datetime.now().date() > datetime.strptime(str(vals['expiry_date']),'%Y-%m-%d').date():
			raise  ValidationError(_('Expiry date have been passed already. Please select a correct expiry date..'))
		if vals.get('issue_date') and  datetime.now().date() > datetime.strptime(str(vals.get('issue_date')),'%Y-%m-%d').date():
			raise  ValidationError(_('Voucher Applicable date is not correct. Either you can make voucher applicable from today or from a future date.'))
		if vals.get('redeemption_limit') and  vals.get('redeemption_limit')== 0:
			raise  ValidationError(_('You cannnot set Reedemption limit To 0'))
		if vals.get('voucher_val_type')== 'percent':
			if vals.get('voucher_value') < 0 or vals.get('voucher_value') >100:
				raise  ValidationError(_('The percentage value should be within 0 and 100'))
			...
			if vals.get('validity', False) or vals.get('issue_date',False):
				...
				if vals.get('validity') > 0:
					...
					if max_expiry_date and  vals['expiry_date'] < datetime.strptime(str(max_expiry_date),'%Y-%m-%d').date():
						vals['expiry_date'] = max_expiry_date
			if vals.get('validity') == 0:
				raise ValidationError(_('Validity can`t be 0. Choose -1 for unlimited or greater than 0 !!!'))
