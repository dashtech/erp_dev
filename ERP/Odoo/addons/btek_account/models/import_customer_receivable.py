...
class ImportCustomerReceivable(models.TransientModel):
    _name = 'wizard.import.customer.receivable'
    ...
    template_file_url = fields.Char(compute='_compute_template_file_url',
                                    ...
                                    'import_customer_receivable.xlsx')

    @api.multi
    def _compute_template_file_url(self):
        base_url = self.env['ir.config_parameter'].get_param('web.base.url')
        url = base_url + '/btek_account/static/import_template/import_customer_receivable.xlsx'
        for ip in self:
            ip.template_file_url = url

    @api.multi
    def import_customer_receivable(self):
        # read excel file
        data = self.datas
        sheet = False

        user = self.env.user
        path = '/import_customer_receivable_template_' \
               + user.login + '_' + str(
            self[0].id) + '_' + str(
            datetime.now()) + '.xls'
        path = path.replace(' ', '_')
    ...
    def verify_excel_data(self, excel_data):
        ...
        for row in excel_data:
            ...
            if not row[partner_code_index]:
                raise UserError(
                    _('Error: row {} partner code invalid!').format(row_count))
    ...
    def get_value_from_excel_row(self, row, row_num, partner_dict, account_dict, currency_dict):
        ...
        if not partner:
            raise UserError(
                _('Error: row {}: cannot find partner with partner code ="{}"'
                  ).format(row_num, partner_code))
        partner_id = partner['id']
        ref = u'Công nợ khách hàng: {}; mã {}; tài khoản {}'.format(
